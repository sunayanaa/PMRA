# ==============================================================================
# Program Name: pmra_04_masterpiece_evaluation.py
# Version: 1.6
# Description: Computes psychoacoustic mask residuals, extracts features, and
#              trains an Isolation Forest to detect backdoored audio.
#              *UPGRADED*: Adds Spectral Flatness to detect artificial structure
#              and implements dynamic Precision-Recall threshold optimization.
...
print("-" * 50)
print("Starting MASTERPIECE PMRA Feature Extraction...")
print("-" * 50)
...
# 4. ADVANCED Psychoacoustic Modeling (PMRA Core)
def compute_residual_features(y, sr):
    ...
    # UPGRADE: Feature 5: Spectral Flatness (Detects artificial tone structure vs noise)
    # A low flatness means it's highly tonal (artificial). High flatness means noise (natural).
    flatness = np.mean(librosa.feature.spectral_flatness(S=residuals+1e-10))
    
    return [peak_mag, centroid, sparsity, flux, flatness]

# 5. Load Manifest and Process Audio
...
for idx, manifest_path in enumerate(eval_paths):
    ...
    if (idx + 1) % 500 == 0:
        print(f"     ... Processed {idx + 1}/{EVAL_SAMPLE_LIMIT} files")

# 6. Isolation Forest & Threshold Optimization
print("\n[INFO] Scaling and training Isolation Forest...")
...
if len(X_poisoned) == 0:
    ...
    y_test = np.array([0]*len(X_poisoned_scaled) + [1]*len(X_poisoned_scaled)) # 1 = Anomaly

    # Score samples (invert so higher is more anomalous)
    y_scores = -iso_forest.score_samples(X_test) 

    # Calculate AUROC
    auroc = roc_auc_score(y_test, y_scores) 

    # UPGRADE: Dynamic Threshold Optimization for Best F1
    ...
    print("-" * 50)
    print("MASTERPIECE PMRA Metrics:")
    print(f"AUROC: {auroc:.4f}")
    print(f"Optimal F1-Score: {optimal_f1:.4f} (at threshold {optimal_threshold:.4f})")
    print("-" * 50)

    results = {"auroc": auroc, "f1_optimal": optimal_f1}
    with open(os.path.join(JSON_OUT_DIR, "pmra_masterpiece_metrics.json"), 'w') as f:
        json.dump(results, f)
    print(f"[SUCCESS] Masterpiece metrics saved.")

print("[COMPLETED] Program 4.5 Finished.")